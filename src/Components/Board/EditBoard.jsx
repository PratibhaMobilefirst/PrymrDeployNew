...
const EditBoard = ({ cameraImage }) => {
  ...
  const { createBlankCanvas, editedImage } = location.state || {};
  ...
  const imageUrl =
    editedImage || JSON.parse(sessionStorage.getItem("state"))?.imageUrl;
  ...
  const handleRemoveTappableArea = (id) => {
    setTappableAreas((prev) => {
      ...
      saveCanvasAsJson(updatedAreas, layers);
      return updatedAreas;
    });
...
//     setTappableAreas((prev) =>
//       prev.map((area) =>
//         area.id === id ? { ...area, content: capturedContent, isVisible: true } : area
//       )
//     );
...
//   const toggleTappableVisibility = (id) => {
//     setTappableAreas((prev) =>
//       prev.map((area) => (area.id === id ? { ...area, isVisible: !area.isVisible } : area))
//     );
//   };
...
//       setTappableAreas((prev) =>
//         prev.map((area) =>
//           area.id === id ? { ...area, size: { width: 50, height: 50 }, isMinimized: true } : area
//         )
//       );
//     } else {
//       handleFixTappableContent(id, position, size);
...
//       prev.map((area) =>
//         area.id === id
//           ? { ...area, isVisible: true, isMinimized: false, size: area.initialSize }
//           : area
//       )
//     );
//     setSelectedLayerId(id);
