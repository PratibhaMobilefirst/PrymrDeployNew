...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      try {
        ...
        // Note: Based on the API error, it seems we don't need to send initialProfileIcon
        // So we'll comment out this step for now
        ...
        if (createUserResponse.data.status) {
          ...
          localStorage.setItem("profileIcon", profileIconUrl);
          localStorage.setItem("initialIconUrl", initialIconUrl);
  ...
  const handleErrorResponse = (data) => {
    ...
    if (data.data.message) {
      newErrors.general = data.data.message;
    }
    if (data.data.data && Array.isArray(data.data.data)) {
      data.data.dataforEach((error) => {
